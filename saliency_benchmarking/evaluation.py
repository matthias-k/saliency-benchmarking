...
class Benchmark(object):
    ...
    def __init__(self, stimuli, fixations, saliency_map_provider):
        self.stimuli = stimuli
        self.fixations = fixations
        self.saliency_map_provider = saliency_map_provider
        self.empirical_maps = pysaliency.FixationMap(stimuli, fixations, kernel_size=24.99)

    def evaluate_model(self, model):
        return {metric_name: self.evaluate_metric(metric_name, model) for metric_name in self.metrics}
    ...
    def evaluate_AUC(self, model):
        return model.AUC(self.stimuli, self.fixations, verbose=True)

    def evaluate_sAUC(self, model):
        return model.AUC(self.stimuli, self.fixations, verbose=True, nonfixations='shuffled')

    def evaluate_NSS(self, model):
        return model.NSS(self.stimuli, self.fixations, verbose=True)
...
class MIT300(Benchmark):
    def __init__(self, dataset_location, fixation_file):
        ...
        super(MIT300, self).__init__(stimuli, fixations, saliency_map_provider)
